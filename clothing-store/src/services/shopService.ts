...
import { Shop, CreateShopRequest, UpdateShopRequest, ShopFilters, ShopStats } from '@/types/shop';
...
export class ShopService {
  static async createShop(shopData: CreateShopRequest, userId: string): Promise<Shop> {
  ...
  static async getShopsWithFilters(filters: ShopFilters): Promise<Shop[]> {
    if (!db || !isFirebaseConfigured) {
      ...
      if (filters.city) {
        shops = shops.filter(shop => shop.city.toLowerCase().includes(filters.city!.toLowerCase()));
      }
      if (filters.township) {
        shops = shops.filter(shop => shop.township.toLowerCase().includes(filters.township!.toLowerCase()));
      }
      if (filters.search) {
  ...
  static async updateShop(id: string, updates: UpdateShopRequest): Promise<void> {
    if (!db || !isFirebaseConfigured) {
      throw new Error('Firebase is not configured');
    }
